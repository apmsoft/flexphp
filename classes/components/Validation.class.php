...
class Validation
{
    ...
    {
        match($column['name']){
            "ID","VIEW_COUNT" => (new FormValidation($column['value'], $column['label'],$value))->null()->number(),
            "NAME" => (new FormValidation($column['value'], $column['label'],$value))->null()->space()->disliking([]),
            "USERID" => (new FormValidation($column['value'], $column['label'],$value))->null()->length(4,16)->space()->disliking()->alnum(),
            ...
            "BIRTHDAY","START_DATE","END_DATE" => (new FormValidation($column['value'], $column['label'],$value))->null()->space()->datef(),
