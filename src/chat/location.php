...
# 미션 정보
$misson_info = $db->get_record('id,msg,misson_way,lat,lon,linkurl,misson_token,extract_id,dvmac', R::$tables['misson'], sprintf("`evtid`='%u' AND `id`='%u'", $req->evtid, $req->missonid));
if(!isset($misson_info['id'])){
    out_json(array('result'=>'false','msg_code'=>'e_db_unenabled', 'msg'=>R::$sysmsg['e_db_unenabled']));
}

if($misson_info['misson_way'] == 'location')
{
