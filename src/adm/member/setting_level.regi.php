...
# 폼및 request값 체크
$form = new ReqForm();
$form->chkAlphabet('feature', 'feature', $req->feature, true);
$form->chkNumber('id', 'id', $req->id, true);
$form->chkNull('value', 'value', $req->v, true);
...
if($req->feature == 'field') {
    R::$r->array['level'][$req->id] = $req->v;
}

# adm log
Log::init(R::$tables['adm_log']);
Log::d( sprintf("MEMBER LEVEL ADD LEVEL ID : %d", $req->id) );
