...
# 기존에 있는 디렉토리 및 파일 삭제
$server_dir = _CONFIG_.DIRECTORY_SEPARATOR.$req->manifid;
...
if($dirObject->isDir(_ROOT_PATH_.DIRECTORY_SEPARATOR.$server_dir)){
    ...
    {
        # 파일삭제
        $del_filename = R::$manifest[$req->manifid]['config'][$req->cfid];
		foreach($config_files as $fn=> $fname){
            if($fname == $del_filename.'.json'){
...
$utilResFtpHelper2 = new UtilResFtpHelper(_RES_, 'manifest');
...
# 쓰기
if(isset(R::$manifest[$req->manifid]['config'][$req->cfid])){
    unset(R::$manifest[$req->manifid]['config'][$req->cfid]);
...
# layout adm docs
if(count(R::$manifest[$req->manifid]['config']) < 1)
{
    # docs 문서가 있는지 체크
    if(file_exists(_ROOT_PATH_.'/'._LAYOUT_.'/docs/'.$req->manifid.'.json')){
        if(!$ftp->ftp_delete(_FTP_DIR_._LAYOUT_.'/docs/'.$req->manifid.'.json')){
            out_json(array('result'=>'false', 'msg'=>'file delete failed!'));
        }
    }
}
