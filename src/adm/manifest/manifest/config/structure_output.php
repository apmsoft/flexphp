...
if($req->feature_column && $req->feature_column !=''){
    $column_value = R::$config['output'][$req->feature_column];
}
...
$data_scheme = array();
$config_data = array();
if(is_array(R::$config['data'])){
    $temp_data = array_keys(R::$config['data']);
    foreach($temp_data as $cdk => $cdv){
        $config_data[$cdk] = $cdv.'.fetch()|count';
        $data_scheme[$cdk] = $cdv;
    }
}
...
# config_model data
$config_model = array();
if(is_array(R::$config['model'])){
    $config_model = array_keys(R::$config['model']);
    if($req->cfid =='list'){
        $config_model[] = 'paging_relation';
        $config_model[] = 'totalPage';
    }
}
...
out_json(array(
    ...
    'funs'         => $myfuns,
    'magic_funs'   => $utilConfigCompiler->magicfuns,
