...
$form->chkEngNumUnderline('after_id', '퀄럼(After) 명', $req->after_id, true);
...
# 실제 테이블 및 퀄럼이 있는지 체크
$column_qry = sprintf(
    "SELECT COLUMN_NAME,COLUMN_TYPE,COLUMN_COMMENT FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name ='%s' AND COLUMN_NAME='%s'", 
        $model->table,
            $req->column_name
        );
if(!$column_rlt = $db->query($column_qry)){
    out_json(array('result'=>'false','msg_code'=>'e_db_unenabled','msg'=>R::$sysmsg['e_db_unenabled']));
}

$column_info = $column_rlt->fetch_assoc();
if(!isset($column_info['COLUMN_NAME'])){
    out_json(array('result'=>'false','msg_code'=>'e_db_unenabled','msg'=>R::$sysmsg['e_db_unenabled']));
}

# 테이블 복사 및 새로 만들기
$column_position_qry = sprintf("ALTER TABLE `%s` MODIFY `%s` %s COMMENT '%s' AFTER `%s`", 
    $model->table, 
        $req->column_name,
