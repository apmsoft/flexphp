...
$form->chkEngNumUnderline('chg_column_name', '새 퀄럼 명', $req->chg_column_name, true);
...
if($req->is_null == 'NOT NULL')
{
    ...
    switch($req->data_type){
        ...
        case 'DOUBLE':
        case 'BIGINT':
            $form->chkNull('data_default', 'Data Default Value', $req->data_default, true);
        break;
    }
...
$change_qry = sprintf("ALTER TABLE `%s` CHANGE COLUMN `%s` `%s` %s%s %s %s COMMENT '%s'", 
    $model->table,
        $req->column_name,
            $req->chg_column_name, 
                $req->data_type,
