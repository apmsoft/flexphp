...
# 폼및 request값 체크
$form = new ReqForm();
$form->chkNumber('muid', '회원번호',$req->muid, true);
...
# 회원 체크
$is_user = $db->get_record('id,userid,passwd,name',R::$tables['member'],sprintf("`id`='%s'",$req->muid));
if(!isset($is_user['id'])){
	out_json(array('result'=>'false','msg_code'=>'e_db_unenabled','msg'=>R::$sysmsg['e_db_unenabled']));
}

# 관리자 체크
$is_adm = $db->get_record('id',R::$tables['admmem'],sprintf("`userid`='%s'",$is_user['userid']));
if(isset($is_adm['id'])){
...
$db['id']       = $is_user['id'];
$db['signdate'] = time();
$db['userid']   = $is_user['userid'];
$db['passwd']   = $is_user['passwd'];
$db['name']     = $is_user['name'];
$db['email']    = $is_user['userid'];
$db['level']    = _AUTH_SUPERADMIN_LEVEL;
$db->insert(R::$tables['admmem']);

# 회원등급업데이트
$db['level'] = _AUTH_SUPERADMIN_LEVEL;
$db->update(R::$tables['member'],sprintf("`id`='%s'",$req->muid));

# adm log
Log::init(R::$tables['adm_log']);
Log::d( sprintf("추가 ADM 아이디(%s) 이름(%s) ", $is_user['userid'],$is_user['name']) );
