...
# config
include_once $path.DIRECTORY_SEPARATOR._SRC_.DIRECTORY_SEPARATOR.'adm'.DIRECTORY_SEPARATOR.'values'.DIRECTORY_SEPARATOR.'strings'.DIRECTORY_SEPARATOR.'config.php';

# Model
$model = new UtilModel();
$model->chkfilename = _ROOT_PATH_.DIRECTORY_SEPARATOR._DATA_.DIRECTORY_SEPARATOR."strings_"._LANG_.'.json';
$model->filename = (file_exists($model->chkfilename)) ? $model->chkfilename : 'strings';
...
# resource
R::init(_LANG_);
R::parserResourceDefinedID('strings');

# check
if(!isset(R::$strings[$req->vkey])){
    out_json(array('result'=>'false','msg_code'=>'e_db_unenabled','msg'=>R::$sysmsg['e_db_unenabled']));
}

# field
R::$strings[$req->vkey] = $req->vval;
...
{
    # 쓰기
    $context = json_encode(R::$strings,JSON_UNESCAPED_UNICODE);
