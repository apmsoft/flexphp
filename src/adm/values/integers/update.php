...
$form->chkNumber('vval', 'value', $req->vval, true);

# config
include_once $path.DIRECTORY_SEPARATOR._SRC_.DIRECTORY_SEPARATOR.'adm'.DIRECTORY_SEPARATOR.'values'.DIRECTORY_SEPARATOR.'integers'.DIRECTORY_SEPARATOR.'config.php';

# Model
$model = new UtilModel();
$model->chkfilename = _ROOT_PATH_.DIRECTORY_SEPARATOR._DATA_.DIRECTORY_SEPARATOR."integers_"._LANG_.'.json';
$model->filename = (file_exists($model->chkfilename)) ? $model->chkfilename : 'integers';
...
# resource
R::init(_LANG_);
R::parserResourceDefinedID('integers');

# check
if(!isset(R::$integers[$req->vkey])){
    out_json(array('result'=>'false','msg_code'=>'e_db_unenabled','msg'=>R::$sysmsg['e_db_unenabled']));
}

# field
R::$integers[$req->vkey] = $req->vval;
...
{
    # 쓰기
    $context = json_encode(R::$integers);
