...
try{
    ...
    #===========[한줄코딩]============;
    # 파일 컨테츠만 가져오기;
    // $file_contents = (new \Flex\Annona\File\Download( $model->picture ))->getContents();

    # 허용된 확장자만
    // $file_contents = (new \Flex\Annona\File\Download( $model->picture ))->setFileTypes(['pdf'])->getContents();
    #Log::d('file contents', $file_contents);
    
    # 다운로드 실행 # 파일 다운로드 실행 (이전에 어떠한 문자도 출력이 되어선 안됨);
    // (new \Flex\Annona\File\Download( $model->picture ))->download('테스트');
    // (new \Flex\Annona\File\Download( $model->picture ))->setFileTypes(['pdf'])->download('테스트');
