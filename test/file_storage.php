...
# 파일 쓰기 1
(new Storage($model->filename, 'w'))->write($model->contents);

# 파일 읽기 1
$read_contents = (new Storage($model->filename, 'r'))->read();
Log::d( $read_contents );

# 파일 쓰기 2 : 한번에 쓰기
(new Storage($model->filename2, 'w'))->put($model->pg_contents);

# 파일 읽기 2 : 한번에 읽기
$get_contents = (new Storage($model->filename2, 'r'))->get();
Log::d($get_contents);

# 파일 쓰기 3 : CSV
(new Storage($model->filename3, 'w'))->write_csv($model->csv_contents);

# 파일 읽기 3 : CSV
$read_csv = (new Storage($model->filename3, 'r'))->read_csv();
Log::d( $read_csv );
?>
