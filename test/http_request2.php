...
try{
    ...
    ->set(
        'https://search.naver.com/search.naver?display=15&f=&filetype=0&page=2&query=flexup&research_url=&sm=tab_pge&start=1&where=web',
        http_build_query(["q"=>"노래방"])
    )
    ->get(function($data) use ($urls){
        if(is_array($data)){
