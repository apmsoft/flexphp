...
function onReady($, _, Backbone) {
    ...
    app.initialize({
        "adm_qqueries" : {
            "list" : {
                ...
                "title":null,
                "frame": null,
                "template": "/_adm/coconut/query/list#tpl_query_listview",
                "value": app.src+"/adm/query/queries/list"
            },
            "edit" : {
                "panel": "bottom",
                "title":"수정",
                ...
                "value": app.src+"/adm/query/queries/edit"
            },
            "post" : {
                "panel": "bottom",
                "title":"등록",
    ...
    _.extend(Activity, {
        ...
        {
            // contents
            var QQueriesActivity = {
                doList: function(params) {
                    var self = this;
    
                    var send_params = {
                        doc_id: 'adm_qqueries/list',
                    ...
                    // panel
                    var panel_setting = app.docs.adm_qqueries.list; // SETTING VALUE
                            ...
                            $(panel.id+'_docs_contents').html(tpl).promise().done(function() 
                            {
                                ...
                                $(panel.id+'_docs_contents .raw-del').on('click', function() 
                                {
                                    ...
                                    if (!_.isUndefined(this_data_id)) {
                                        ...
                                        if(cf){
                                            var send_params = {
                                                doc_id: "adm_qqueries/delete",
                                                id : this_data_id
                                            };
                                            _.extend(send_params, params);
                            
                                            ProgressBar.show_progress();
                                            DocAsyncTask.doPostMessage(app.src+"/adm/query/queries/delete", send_params, {
                ...
                doPost: function() {
                    var self = this;
                    var send_params = {
                        doc_id: 'adm_qqueries/post'
                    };
    
                    // panel
                    var panel_setting = app.docs.adm_qqueries.post; // SETTING VALUE
                    var panel = Panel.onStart(panel_setting);
                        ...
                        success: function(tpl, resp) {
                            $(panel.id+'_docs_contents').html(tpl).promise().done(function() {
                                ...
                                DocAsyncTask.doSubmit(panel.id+'_docs_contents #theDefaultForm', function(form_params)
                                {
                                    ...
                                    var _send_params = {
                                        doc_id : 'adm_qqueries/insert'
                                    };
                                    _.extend(_send_params,form_params);

                                    DocAsyncTask.doPostMessage(app.src+"/adm/query/queries/insert", _send_params, {
                                        success : function(resp){
                                            ProgressBar.close_progress();
                                            Toast.show('',resp.msg,2000, {style:'success'});
                ...
                doEdit: function(params) {
                    var self = this;
                    var send_params = {
                        doc_id: "adm_qqueries/edit",
                        cache :false
                    };
                    _.extend(send_params, params);
    
                    // panel
                    var panel_setting = app.docs.adm_qqueries.edit; // SETTING VALUE
                    var panel = Panel.onStart(panel_setting);
                        ...
                        success: function(tpl, resp) {
                            ...
                            $(panel.id+'_docs_contents').html(tpl).promise().done(function() 
                            {
                                ...
                                DocAsyncTask.doSubmit(panel.id+'_docs_contents #theDefaultForm', function(form_params)
                                {
                                    ...
                                    var _send_params = {
                                        doc_id : 'adm_qqueries/update'
                                    };
                                    _.extend(_send_params,form_params);

                                    DocAsyncTask.doPostMessage(app.src+"/adm/query/queries/update", _send_params, {
                                        success : function(resp){
                                            ProgressBar.close_progress();
                                            Toast.show('',resp.msg,2000, {style:'success'});
            ...
            QQueriesActivity.doList(UrlUtil._url_params);
        }
    });
