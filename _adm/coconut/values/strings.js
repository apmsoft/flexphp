...
function onReady($, _, Backbone) {
    ...
    app.initialize({
        "adm_vstrings" : {
            "list" : {
                ...
                "value": app.src+"/adm/values/strings/list"
            },
            "edit" : {
                "panel": "bottom",
                "title":"수정",
                ...
                "value": app.src+"/adm/values/strings/edit"
            },
            "post" : {
                "panel": "bottom",
                "title":"등록",
    ...
    _.extend(Activity, {
        ...
        {
            // contents
            var VStringsActivity = {
                doList: function(params) {
                    var self = this;
    
                    var send_params = {
                        doc_id: 'adm_vstrings/list',
                    ...
                    // panel
                    var panel_setting = app.docs.adm_vstrings.list; // SETTING VALUE
                            ...
                            $(panel.id+'_docs_contents').html(tpl).promise().done(function() 
                            {
                                ...
                                $(panel.id+'_docs_contents .raw-del').on('click', function() 
                                {
                                    ...
                                    if (!_.isUndefined(this_data_id)) {
                                        ...
                                        if(cf){
                                            var send_params = {
                                                doc_id: "adm_vstrings/delete",
                                                id : this_data_id
                                            };
                                            _.extend(send_params, params);
                            
                                            ProgressBar.show_progress();
                                            DocAsyncTask.doPostMessage(app.src+"/adm/values/strings/delete", send_params, {
                ...
                doPost: function() {
                    var self = this;
                    var send_params = {
                        doc_id: 'adm_vstrings/post'
                    };
    
                    // panel
                    var panel_setting = app.docs.adm_vstrings.post; // SETTING VALUE
                    var panel = Panel.onStart(panel_setting);
                        ...
                        success: function(tpl, resp) {
                            $(panel.id+'_docs_contents').html(tpl).promise().done(function() {
                                ...
                                DocAsyncTask.doSubmit(panel.id+'_docs_contents #theDefaultForm', function(form_params)
                                {
                                    ...
                                    var _send_params = {
                                        doc_id : 'adm_vstrings/insert'
                                    };
                                    _.extend(_send_params,form_params);

                                    DocAsyncTask.doPostMessage(app.src+"/adm/values/strings/insert", _send_params, {
                                        success : function(resp){
                                            ProgressBar.close_progress();
                                            Toast.show('',resp.msg,2000, {style:'success'});
                ...
                doEdit: function(params) {
                    var self = this;
                    var send_params = {
                        doc_id: "adm_vstrings/edit",
                        cache :false
                    };
                    _.extend(send_params, params);
    
                    // panel
                    var panel_setting = app.docs.adm_vstrings.edit; // SETTING VALUE
                    var panel = Panel.onStart(panel_setting);
                        ...
                        success: function(tpl, resp) {
                            ...
                            $(panel.id+'_docs_contents').html(tpl).promise().done(function() 
                            {
                                ...
                                DocAsyncTask.doSubmit(panel.id+'_docs_contents #theDefaultForm', function(form_params)
                                {
                                    ...
                                    var _send_params = {
                                        doc_id : 'adm_vstrings/update'
                                    };
                                    _.extend(_send_params,form_params);

                                    DocAsyncTask.doPostMessage(app.src+"/adm/values/strings/update", _send_params, {
                                        success : function(resp){
                                            ProgressBar.close_progress();
                                            Toast.show('',resp.msg,2000, {style:'success'});
            ...
            VStringsActivity.doList(UrlUtil._url_params);
        }
    });
