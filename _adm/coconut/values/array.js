    ...
    app.initialize({
        "adm_varray" : {
            "list" : {
                ...
                "title":null,
                "frame": null,
                "template": "/_adm/coconut/values/list_array#tpl_values_listarray",
                "value": app.src+"/adm/values/array/list"
            },
            "edit" : {
                "panel": "bottom",
                "title":"수정",
                ...
                "value": app.src+"/adm/values/array/edit"
            },
            "post" : {
                "panel": "bottom",
                "title":"등록",
        ...
        onCreateView: function() 
        {
            ...
            var VArrayActivity = {
                doList: function(params) {
                    ...
                    // panel
                    var panel_setting = app.docs.adm_varray.list; // SETTING VALUE
                            ...
                            $(panel.id+'_docs_contents').html(tpl).promise().done(function() 
                            {
                                ...
                                $(panel.id+'_docs_contents .raw-del').on('click', function() 
                                {
                                    ...
                                    if (!_.isUndefined(this_data_id)) {
                                        ...
                                        if(cf){
                                            var send_params = {
                                                doc_id: "adm_varray/delete",
                                                id : this_data_id
                                            };
                                            _.extend(send_params, params);
                            
                                            ProgressBar.show_progress();
                                            DocAsyncTask.doPostMessage(app.src+"/adm/values/array/delete", send_params, {
                ...
                doPost: function() {
                    var self = this;
                    var send_params = {
                        doc_id: 'adm_varray/post'
                    };
    
                    // panel
                    var panel_setting = app.docs.adm_varray.post; // SETTING VALUE
                    var panel = Panel.onStart(panel_setting);
                        ...
                        success: function(tpl, resp) {
                            $(panel.id+'_docs_contents').html(tpl).promise().done(function() {
                                ...
                                DocAsyncTask.doSubmit(panel.id+'_docs_contents #theDefaultForm', function(form_params)
                                {
                                    ...
                                    var _send_params = {
                                        doc_id : 'adm_varray/insert'
                                    };
                                    _.extend(_send_params,form_params);

                                    DocAsyncTask.doPostMessage(app.src+"/adm/values/array/insert", _send_params, {
                                        success : function(resp){
                                            ProgressBar.close_progress();
                                            Toast.show('',resp.msg,2000, {style:'success'});
                ...
                doEdit: function(params) {
                    var self = this;
                    var send_params = {
                        doc_id: "adm_varray/edit",
                        cache :false
                    };
                    _.extend(send_params, params);
    
                    // panel
                    var panel_setting = app.docs.adm_varray.edit; // SETTING VALUE
                    var panel = Panel.onStart(panel_setting);
                        ...
                        success: function(tpl, resp) {
                            ...
                            $(panel.id+'_docs_contents').html(tpl).promise().done(function() 
                            {
                                ...
                                DocAsyncTask.doSubmit(panel.id+'_docs_contents #theDefaultForm', function(form_params)
                                {
                                    ...
                                    var _send_params = {
                                        doc_id : 'adm_varray/update'
                                    };
                                    _.extend(_send_params,form_params);

                                    DocAsyncTask.doPostMessage(app.src+"/adm/values/array/update", _send_params, {
                                        success : function(resp){
                                            ProgressBar.close_progress();
                                            Toast.show('',resp.msg,2000, {style:'success'});
            ...
            VArrayActivity.doList(UrlUtil._url_params);
        }
    });
