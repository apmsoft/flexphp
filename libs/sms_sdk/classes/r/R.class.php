...
final class R
{
            ...
            foreach($resources as $resource_path => $resouces_args){
                foreach($resouces_args as $resource_name){
                    if(property_exists(__CLASS__,$resource_name)){
                        self::parserResource(self::findLanguageFile(_SMS_ROOT_PATH_.'/'.$resource_path.'/'.$resource_name.'.json'), $resource_name);
                    }
                }
            }
        }
    ...
    public static function parserResourceDefinedID($query){
        ...
        switch($query){
            ...
            case 'floats':
            case 'doubles': 
                self::parserResource(self::findLanguageFile(_SMS_ROOT_PATH_.'/'._VALUES_.'/'.$query.'.json'), $query);
            break;
            case 'tables':
            case 'queries':
                self::parserResource(self::findLanguageFile(_SMS_ROOT_PATH_.'/'._QUERY_.'/'.$query.'.json'), $query);
            break;
            case 'layout':
                self::parserResource(self::findLanguageFile(_SMS_ROOT_PATH_.'/'._LAYOUT_.'/'.$query.'.json'), $query);
            break;
            case 'manifest':
                self::parserResource(self::findLanguageFile(_SMS_ROOT_PATH_.'/'._RES_.'/'.$query.'.json'), $query);
            break;
            case 'config':
                self::parserResource(self::findLanguageFile(_SMS_ROOT_PATH_.'/'._CONFIG_.'/'.$query.'.json'), $query);
            break;
        }
    }

    #@ void
    # R::parserResource(_SMS_ROOT_PATH_.'/'._QUERY_.'/queries.json', 'queries');
    # out_r(R::$queries);
